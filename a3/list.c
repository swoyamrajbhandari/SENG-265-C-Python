 ...
 *
 */
node_t *new_node(char *artist, char* song, char* year, char* sortby)
{
    assert(artist != NULL);
    assert(song != NULL);
    assert(year != NULL);
    ...
    temp->year = strdup(year);
    temp->sortby = strdup(sortby);
...
node_t *add_inorder(node_t *list, node_t *new)
{
    ...
    {
        if (strcmp(new->sortby, curr->sortby) < 0)
        {
            prev = curr;
        }
        else if (strcmp(new->sortby, curr->sortby) == 0)
        {
            if (strcmp(new->song, curr->song) < 0){
                prev = curr;
            } else {
                break;
            }
